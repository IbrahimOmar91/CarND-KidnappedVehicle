...
int main()
{
  ...
  double delta_t = 0.1;     // Time between measurements
  double sensor_range = 50; // Sensor range

  double sigma_pos [3] = {0.3, 0.3, 0.01}; // GPS measurement uncertainty [x, y, theta]
  ...
  h.onMessage([&pf,&map,&delta_t,&sensor_range,&sigma_pos,&sigma_landmark](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
    if (length && length > 2 && data[0] == '4' && data[1] == '2')
    {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          if (!pf.initialized()) {
          	// Sense pos data from the simulator
		  ...
		  // Calculate avg weighted error since start
		  ...
		  cout << setprecision(5) << "Weight >> Max: " << max_weight << "\tAvg: " << weight_sum/num_particles << endl;
